...
JA_OVERRIDES: list[tuple[str, str]] = [
 ...
 ("Cells that build the hippocampal extracellular environment",
  ...
  "leave-one-donor-out再推定でも符号を保持し、検定可能なまま残った57ペアはすべてアンビエントRNA補正後も方向を保持し、"
 ...
 ("Here, we reanalysed a human hippocampal single-nucleus multiome",
  "本研究では、若年成人期、臨床的に対照と定義された加齢、アルツハイマー病にまたがるヒト海馬の"
  "単一核マルチオームを再解析した [17]。互いに関連する4つの問いを立てた。すなわち、確立された"
  "ニッチ支持細胞において加齢とともに変化するセクレトーム関連転写産物はどれか。同じプログラムは"
  "同一ドナーの成熟ニューロンにも現れるか。その方向は独立したヒトのデータセットおよびタンパク質"
  "参照で再現するか。そして、利用可能なデータはどの制御機構・空間的機構を支持するか、である。"
  "反復の単位は核ではなくドナーとし、加齢シグネチャーはアルツハイマー病との比較を用いずに定義した。"
  "さらに、デザインそのものも測定の対象として扱った。一次コントラストはドナー群ラベル置換によって"
  "較正し、各主張には、それを破ることを目的とした対照を対置した。これにより、このデータが支えられる"
  "推論と支えられない推論とを、有意遺伝子の個数から推し量るのではなく切り分けている。"
  "得られた枠組みは意図的に神経新生に依存しない。成体神経新生が存在しない、あるいは重要でないと"
  "主張するものではなく、その問いを先に解決しなくても研究できる、支持細胞に偏った海馬加齢の側面を"
  "明らかにするものである。"),

 ("This study identifies an age-associated shift in the transcriptional program",
  "本研究は、ヒト海馬の複数の支持細胞区画において、細胞外タンパク質をコードする転写プログラムが"
  "加齢に伴って変化することを示した。このシグナルは、一つの強いマーカーや一つの細胞集団だけで"
  "生じたものではない。アストロサイトにおける細胞外マトリックス・マトリセルラー関連転写産物の増加、"
  "ミクログリアにおける炎症性転写産物の増加、オリゴデンドロサイト系譜の変化、選択された血管系因子の"
  "低下が組み合わさっていた。60の遺伝子×細胞型ペアは、個々のドナーを除外してもすべて方向を保ち、"
  "5つの支持細胞区画のうち4つでは、ドナー群ラベルを置換した場合より多くのセクレトーム関連変化が"
  "得られた——うち3つはゲノムワイドの検出数でも同様であった。したがって、これは一つの大きな状態転換"
  "というより、局所組織環境を構成する複数の細胞が協調して、しかし比較的穏やかに変化する像である。"),
 ...
 ("Deposited single-nucleus data now supply much of what is claimed",
  ...
  "その較正を、主要な比較が出そろったここで報告するのは、本稿のあらゆる検出数をどう読むべきかを"
  "定めるのがこの較正だからである。"),
 ...
 ("Multiple-testing scope was fixed in advance",
  ...
  "5つのニッチ細胞クラスにまたがる99,544検定は1つではなく5つの検定ファミリーを構成する（再生成した表では99,543件。方法の「再現可能性」を参照）。5クラスを単一の"
 ...
 ("All primary and secondary datasets are publicly available",
  ...
  "データセット [15]、CSF加齢プロテオーム [16]、NicheNet v2事前分布（https://doi.org/10.5281/zenodo.7074291）[31]、"
  ...
  "ドナーラベル帰無分布を含む）を、それぞれの由来記録とともに収めてある。このアーカイブは "
  "{ZENODO_DOI} に寄託し、{REPO_URL} で開発している。"),
 ...
 ("The analysis code, the pre-specified analysis documents, and the result tables that support",
  ...
  "含むR環境）、特記しない限り乱数シードは42を用い、コードはMITライセンスで公開する。図、検証、補足表、多重検定スコープの各ターゲットは、"
...
# The Japanese mirror fills the same placeholders as the English. Import them rather than
# repeating them: a second copy silently keeps the old URL when the English one changes,
# which is how the two versions came to cite different Zenodo DOIs.
sys.path.insert(0, str(Path(__file__).resolve().parent))
from revision_edits import PLACEHOLDERS as PLACEHOLDER_JA  # noqa: E402
