...
# The typeset artefacts are built under submission/v2/ in the working repository. The
# public release carries the same files beside the text of record, under
# manuscript/biorxiv_v2/, because "submission/" names a route this work no longer takes.
# Look in both places so the checks that read them run in either checkout.
def _either(working: str, released: str) -> Path:
    w = PROJ / working
    return w if w.exists() else PROJ / released


DOCX = _either("submission/v2/shimozaki-aging-secretome-v2.docx",
               "manuscript/biorxiv_v2/shimozaki-aging-secretome-v2.docx")
FIGS = _either("submission/v2/figures", "manuscript/biorxiv_v2/figures")
PDF = _either("submission/v2/shimozaki-aging-secretome-v2.pdf",
              "manuscript/biorxiv_v2/shimozaki-aging-secretome-v2.pdf")
...
def main() -> int:
    ...
    # The merged PDF -- manuscript text plus the fifteen composed figures -- is assembled
    # by hand and so cannot be regenerated by build_revision.py. It is the copy readers
    # download, so the addresses it sends them to are checked against the text of record
    # rather than trusted. The rest of its text is not compared: two of the figures carry
    # type converted to outlines and have no text layer at all.
    if not PDF.exists():
        notes.append("merged-PDF checks skipped: the typeset PDF is not part of this "
                     "checkout (it is assembled in the working repository)")
        print("  [skip] merged manuscript PDF not present in this checkout")
    else:
        out = subprocess.run(["pdftotext", "-layout", str(PDF), "-"],
                             capture_output=True, text=True).stdout
        pdf_text = re.sub(r"\s+", " ", out)
        check("merged PDF has 54 pages (39 text + 15 figures)",
              subprocess.run(["pdfinfo", str(PDF)], capture_output=True, text=True)
              .stdout.count("Pages:") == 1 and re.search(
                  r"Pages:\s+(\d+)", subprocess.run(
                      ["pdfinfo", str(PDF)], capture_output=True, text=True).stdout
              ).group(1) == "54")
        urls = sorted({u.rstrip(".,;:)")
                       for u in re.findall(r"https://(?:doi\.org|github\.com)/\S+", en)})
        check("the text of record carries 3 addresses to check", len(urls) == 3,
              " ".join(urls))
        for url in urls:
            check(f"merged PDF cites {url}", url in pdf_text)
        stale = re.findall(r"10\.5281/zenodo\.\d+", pdf_text)
        wanted = set(re.findall(r"10\.5281/zenodo\.\d+", en))
        check("merged PDF names no Zenodo record the text of record does not",
              set(stale) <= wanted, ", ".join(sorted(set(stale) - wanted)))
    ...
    for meta in (".zenodo.json", "CITATION.cff"):
        ...
        # The metadata describes the deposit's layout, where the typeset manuscript and
        # its figures sit beside the text of record. In the working repository they are
        # still under submission/v2/, so accept either place -- the same fallback DOCX
        # and FIGS use above.
        def somewhere(rel: str) -> bool:
            rel = rel.rstrip("/.")
            alt = rel.replace("manuscript/biorxiv_v2/figures", "submission/v2/figures")
            alt = alt.replace("manuscript/biorxiv_v2/shimozaki-", "submission/v2/shimozaki-")
            return (PROJ / rel).exists() or (PROJ / alt).exists()

        missing = sorted(n for n in named if not somewhere(n))
        check(f"{meta} names only paths that exist here", not missing, ", ".join(missing[:5]))
    # The deposit is registered as the preprint of this manuscript, so the title it is
    # registered under has to be the manuscript's own. Both metadata files once said
    # "Analysis code for ..." while the description said the manuscript was published
    # here, which is the sort of disagreement between surfaces that nobody notices.
    title = en.splitlines()[0].lstrip("# ").strip()
    for meta, pattern in ((".zenodo.json", r'"title":\s*"([^"]+)"'),
                          ("CITATION.cff", r"(?m)^title:[ \t]*>-\n((?:  .*\n)+)")):
        f = PROJ / meta
        if not f.exists():
            continue
        m = re.search(pattern, f.read_text(encoding="utf-8"))
        got = re.sub(r"\s+", " ", m.group(1)).strip() if m else ""
        check(f"{meta} is registered under the manuscript's title", got == title, got[:70])
